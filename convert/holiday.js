...
const getNationalHolidays = async () => {
  const from =
    moment().subtract(TARGET_MONTHS_START, 'months').format('YYYY-MM-DD') +
    'T00:00:00Z';
  const to =
    moment().add(TARGET_MONTHS_END, 'months').format('YYYY-MM-DD') +
    'T00:00:00Z';

  // Googleに渡す日付を作成
  try {
    const res = await axios.get(
      `https://www.googleapis.com/calendar/v3/calendars/${ID}/events?key=${API_KEY}&timeMin=${from}&timeMax=${to}`
    );
    //console.log(res)
    ...
    alert(
      `API接続時にエラーが発生しました。HTTP Status: ${status} ${statusText}`
    );
    throw error;
  }
};
...
const getShukkaYoteibi = (values) => {
  ...
  if (!date.isValid()) {
    return getClosestWeekday(moment().startOf('day').add(1, 'days'));
  }
...
const validateShukkaYoteibi = (values) => {
  ...
  if (date.isBefore(moment(), 'day')) {
    throw new Error(
      `お届け希望日から計算した出荷予定日が今日以前（${date.format(
        'YYYY/MM/DD'
      )}）になってしまいます。`
    );
  }
