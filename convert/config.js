...
const {getCustomerCode, getKyouRyokuKaisyaCd, convertRowNum, convertDateFormat} = require('./functions')
const {outputsShipment} = require('./output/outputsShipment')
const {outputsCoupon} = require('./output/outputsCoupon')
const {outputsTax} = require('./output/outputsTax')
...
const validateByteSize = (fromValue, size, fieldName) => {
  var b = 0;
  for (var i = 0; i < fromValue.length; i++) {
    //半角カナ
    var reg = new RegExp(/^[ｦ-ﾟ]*$/);
...
module.exports = {
  inputSettings: {
    ...
    parses: {
      ...
      'HTMLメール受信可否フラグ': (value) => {
        return value
      },
      'モバイルメルマガ受信可否フラグ': (value) => {
        return value
      },
      'アンケート回答内容01': (value) => {
        return value
  ...
  outputSettings: {
    outputs: {
      ...
      columns: [
        ...
        {name: '行摘要２'},
        {name: '備考ｺｰﾄﾞ'},
        {name: '備考', from: ['決済方法', '請求合計金額'],
          convert: (value) => {
            return value[0] == '代金引換' && 300000 <= value[1] ? "代引きで受注合計金額が30万円を超えています" : '';
          }
        },
        {name: '見積処理連番'},
        {name: '見積行'},
        ...
        {name: '納品先郵便番号', from: 'お届け先郵便番号'},
        {
          ...
          convert: (values) => {
            return convertSpecialChars(values[0] + values[1])
          }
        },
    ...
    //送料の出力設定
    outputsShipment: outputsShipment,
    //クーポンの出力設定
    outputsCoupon: outputsCoupon,
    //消費税出力設定
    outputsTax: outputsTax
  },
  validationSettings: [
    {
      field: ['お届け先氏名（姓）', 'お届け先氏名（名）'],
      validation: (values) => {
        const name = convertSpecialChars(values[0] + values[1])
        return validateByteSize(name, 48, 'お届け先氏名（姓）とお届け先氏名（名）の合計')
      },
    },
    {
      field: 'お届け先氏名（姓）',
